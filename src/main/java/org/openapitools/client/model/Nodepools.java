...
import org.openapitools.client.model.NodepoolInstances;
...
public class Nodepools {
  ...
  private List<NodepoolInstances> nodes;
  ...
  * @throws IOException if the JSON Element is invalid with respect to Nodepools
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Nodepools.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Nodepools.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Nodepools` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("nodes") != null && !jsonObj.get("nodes").isJsonNull()) {
        ...
        if (jsonArraynodes != null) {
          ...
          for (int i = 0; i < jsonArraynodes.size(); i++) {
            NodepoolInstances.validateJsonElement(jsonArraynodes.get(i));
          };
        }
      }
