...
public class PlansMetal {
  ...
  * @throws IOException if the JSON Element is invalid with respect to PlansMetal
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PlansMetal.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PlansMetal.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PlansMetal` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("cpu_model") != null && !jsonObj.get("cpu_model").isJsonNull()) && !jsonObj.get("cpu_model").isJsonPrimitive()) {
