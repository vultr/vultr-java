...
public class GetInstanceUpgrades200ResponseUpgrades {
  ...
  * @throws IOException if the JSON Element is invalid with respect to GetInstanceUpgrades200ResponseUpgrades
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GetInstanceUpgrades200ResponseUpgrades.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GetInstanceUpgrades200ResponseUpgrades.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GetInstanceUpgrades200ResponseUpgrades` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
