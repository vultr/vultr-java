...
public class CreateLoadBalancerForwardingRulesRequest {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CreateLoadBalancerForwardingRulesRequest
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreateLoadBalancerForwardingRulesRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreateLoadBalancerForwardingRulesRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreateLoadBalancerForwardingRulesRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("frontend_protocol").isJsonPrimitive()) {
      ...
      if (!jsonObj.get("backend_protocol").isJsonPrimitive()) {
