...
public class RegenerateObjectStorageKeys201ResponseS3Credentials {
  ...
  * @throws IOException if the JSON Element is invalid with respect to RegenerateObjectStorageKeys201ResponseS3Credentials
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RegenerateObjectStorageKeys201ResponseS3Credentials.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RegenerateObjectStorageKeys201ResponseS3Credentials.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `RegenerateObjectStorageKeys201ResponseS3Credentials` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("s3_hostname") != null && !jsonObj.get("s3_hostname").isJsonNull()) && !jsonObj.get("s3_hostname").isJsonPrimitive()) {
      ...
      if ((jsonObj.get("s3_access_key") != null && !jsonObj.get("s3_access_key").isJsonNull()) && !jsonObj.get("s3_access_key").isJsonPrimitive()) {
      ...
      if ((jsonObj.get("s3_secret_key") != null && !jsonObj.get("s3_secret_key").isJsonNull()) && !jsonObj.get("s3_secret_key").isJsonPrimitive()) {
