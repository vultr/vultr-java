...
public class InstanceV6NetworksInner {
  ...
  * @throws IOException if the JSON Element is invalid with respect to InstanceV6NetworksInner
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InstanceV6NetworksInner.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!InstanceV6NetworksInner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InstanceV6NetworksInner` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
